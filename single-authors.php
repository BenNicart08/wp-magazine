...
<section class="mb-20 latest">
      <div class="container">
        ...
        <?php if($magazines) : ?>
            <?php foreach($magazines as $x) : setup_postdata( $x );
            $thumbnail = get_field('thumbnail',$x->ID);
            $title = get_the_title($x->ID);
            $duration = get_field('duration',$x->ID);
            $date = get_the_date('j, F Y',$x->ID);
